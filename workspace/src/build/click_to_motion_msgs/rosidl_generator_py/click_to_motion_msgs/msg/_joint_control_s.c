...
bool click_to_motion_msgs__msg__joint_control__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("click_to_motion_msgs.msg._joint_control.JointControl", full_classname_dest, 52) == 0);
  }
  click_to_motion_msgs__msg__JointControl * ros_message = _ros_message;
  {  // robot_id
  ...
  {  // joint_accelerations
    PyObject * field = PyObject_GetAttrString(_pymsg, "joint_accelerations");
    if (!field) {
      return false;
    }
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->joint_accelerations.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'joint_accelerations'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
PyObject * click_to_motion_msgs__msg__joint_control__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("click_to_motion_msgs.msg._joint_control");
  ...
  {  // joint_accelerations
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "joint_accelerations");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->joint_accelerations.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->joint_accelerations.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->joint_accelerations.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
